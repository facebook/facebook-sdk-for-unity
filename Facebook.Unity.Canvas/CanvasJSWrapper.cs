...
namespace Facebook.Unity.Canvas
{
    ...
    internal class CanvasJSWrapper : ICanvasJSWrapper
    {
        ...
        {
            var scopeList = new List<string>(scope);
            var scopeListJson = MiniJSON.Json.Serialize(scope);

            login(scopeListJson, callback_id);
        }
        ...
        public void LogAppEvent(string eventName, float? valueToSum, string parameters) {
            if (valueToSum.HasValue)
                logAppEvent(eventName, valueToSum.Value, parameters);
            else
                logAppEventWithoutValue(eventName, parameters);
        }
        ...
        [DllImport("__Internal")]
        private static extern void logAppEventWithoutValue(string eventName, string parameters);
