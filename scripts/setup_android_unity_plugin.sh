...
FB_ANDROID_SDK_WRAPPER="$FB_WRAPPER_PATH/build/outputs/aar/facebook-android-wrapper-release.aar"
...
ANDROID_SUPPORT_LIB_PATH="$ANDROID_HOME/extras/android/support/v4/android-support-v4.jar"
...
info "Step 2.1 - Download $BOLTS_SDK_JAR_NAME"
if [ ! -f "$BOLTS_JAR_PATH" ]; then
  downloadFromMaven $BOLTS_GROUP_ID $BOLTS_ARTIFACT_ID $BOLTS_PACKAGING $BOLTS_VERSION "$BOLTS_JAR_PATH"
else
...
if [ "$localBuild" = false ]; then
  ...
  cp $BOLTS_JAR_PATH $UNITY_PLUGIN_FACEBOOK || die 'Failed to copy bolts jar to unity plugin folder'
